...
![A well preserved Bouma sequence from Tabernas Basin, Spain. [reddit
...
The layered mud in the image above depicts a [Bouma
sequence](https://en.wikipedia.org/wiki/Bouma_sequence), which to the untrained
eye, is... whatever man, it's mud. To the trained eye however, it is a dead giveaway
that the sediment was deposited by a series of dramatic submarine landslides, on the
steep continental shelf of a convergent tectonic margin. 
...
Bayesians don't argue about the truth though---they just agree to share it around to the
best candidates.

Coming at this from a bayesian perspective then, it seems like there should be a
framework for creating a distribution over geological interpretations. Speaking about
sedimentary sequences specifically, I think there are tools on the table of bayesian
statistics to use. 

## The Dirichlet Distribution

The [Dirichet distribution](https://en.wikipedia.org/wiki/Dirichlet_distribution) can be
used to model the probability of a set of discrete possibilities---depositional
environments, in this context. The Dirichlet distribution samples from a simplex (a
vector of numbers that add to 1), which makes it perfect for modelling our uncertainty
over interpretations, where we assign some probability to each.

It also has the convenient property of conjugacy, which essentially means it is very
...
very common in bayesian statistics. I will write a post on that at some point, but see
the **Aleatoric and epistemic** section of [this](https://en.wikipedia.org/wiki/Uncertainty_quantification) wiki page for a primer.
...
The Bouma sequence example actually points to a big problem in this simple
...
Again, it wouldn't be easy to calibrate these $\alpha$ values as they are encoding
subjective beliefs about strength of evidence under different interpretations.
...
2. The treating of observations in isolation, instead of considering how they interact
...
So you see... it's a simple matter of setting all those alpha scores for every
depositional environment, all the connections between observations, and all the edge
weights of those connections! Why aren't people doing this already?

# Conclusion

Regardless of some big problems with implementing this approach, I think it shows enough
promise that, if I was working on these sorts of problems in my day-to-day, I would try
to flesh it out in more detail. 
